...
# house-style status markers and medals; Builder Center takes none of them
EMOJI = ("🥇 ", "🥈 ", "🥉 ", "✅ ", "❌ ", "⚠️ ", "🔎 ", "🟢 ",
         "🥇", "🥈", "🥉", "✅", "❌", "⚠️", "🔎", "🟢")
...
def main():
    ...
    left = [e for e in EMOJI if e.strip() and e.strip() in text]
    cols = widest_table(text)
    print(f"{src.name} -> {out.name}  {len(text):,} chars, {len(text.splitlines())} lines")
    fails = 0
    if left:
        print(f"  FAIL  emoji survived the strip: {sorted(set(e.strip() for e in left))}")
        fails += 1
    else:
        print("  ok    no emoji")
    if cols > MAX_COLS:
