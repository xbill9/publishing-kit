...
| `check-prose.py` | Prose lint. Fails on the author's banned phrasing — narrating the work, "X is not Y" contrast lines, sincerity filler, internal jargon — from the `prose-lint` block in `references/house-style.md`. Code blocks and inline code are skipped. Run by `preflight.py`. |
| `check-facts.py` | Fact-tracing against evidence files. Exits non-zero on untraced claims. `.factsignore` records deliberate exemptions with reasons. A claim measured earlier and deliberately not re-run may cite a dated `## LOG` row in the article's ledger (`--ledger`, default `CLAIMS.md` beside it); those print as `LOG <date>` and do not fail the run. The ledger may not sit inside `--evidence`. |
